    ...
    if (schedulePattern[day]) {
      ...
      schedulePattern[day].forEach((period) => {
        ...
        if (type === 'subject' && availableSubjects.length > 0) {
          let flag = false;
          while (!flag && availableSubjects.length > 0) {
            const randomIndex = Math.floor(Math.random() * availableSubjects.length);
            name = availableSubjects[randomIndex].name;
            flag = !periods.some((period) => period.name === name);
            if (flag) {
        ...
        if( type === 'break')
        {
          periods.push({ time, type });
        }
       else
        {
            periods.push({ time, type, name });
        }
